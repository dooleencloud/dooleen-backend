 ...
 * limitations under the License.ww
 */
...
public class DateUtils extends org.apache.commons.lang3.time.DateUtils {
	...
	//private static //Logger //Logger = //Logger.get//Logger(DateUtils.class);
	...
	public static final SimpleDateFormat TIMES_STAMP = new SimpleDateFormat("yyyyMMddHHmmss");
    public DateUtils()
    {}
	/**
	 * 根据日期格式字符串解析日期字符串
	 ...
	 * @return
	 */
	public static String initialDatatime() {
	...
	/**
	 * 取得通用日期时间格式字符串：yyyyMMddHHmmss
	 * 默认当前日期
	 * @param date
	 * @return String
	 */
	public static String getCurrTimestamp(Date date) {
		return TIMES_STAMP.format(date);
	}
	
	/**
	 * 取得通用日期时间格式字符串：yyyy-MM-dd HH:mm:ss
	 * 默认当前日期
	 * @param date
	 * @return String
	 */
	public static String getCurrTimes(Date date) {
		if (null == date) {
			date = new Date();
		}
	...
	/**
	 * @Description: 计算两个日期相差的天数（去除周六周日）
	 * @Title: countWorkDay
	 * @param String, String
	 * @return long
	 */
	public static long countWorkDay(String startDate, String endDate) {
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		Date sDate = new Date();
		try {
		...
		// 总相差天数
		long diffDay = (eDate.getTime()-sDate.getTime()) / ( 1000 * 3600 * 24) + 1;
	    if(diffDay == 0)
	        return diffDay;
	    // 周末天数
	    long weekEnds = 0; 
	...
	/**
	 * @Description: 计算两个日期相差的天数,包含周末
	 * @Title: countDays
	 * @param String, String
	 * @return long
	 */
	public static Integer countDays(String startDate, String endDate) {
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		Date sDate = new Date();
		try {
			sDate = sdf.parse(startDate);
		...
		// 总相差天数
		Integer diffDay = Integer.valueOf(String.valueOf((eDate.getTime()-sDate.getTime()) / (DAY_MILLI) + 1));
		return diffDay;
	}
    ...
    public static List<String> getBetweenDays(Date start, Date end) {
        ...
        while (tempStart.before(tempEnd) 
        		|| (sdf.format(tempStart.getTime()).equals(sdf.format(tempEnd.getTime())) && !"00:00:00".equals(sdfTime.format(tempEnd.getTime()).substring(11, 19)))) {
