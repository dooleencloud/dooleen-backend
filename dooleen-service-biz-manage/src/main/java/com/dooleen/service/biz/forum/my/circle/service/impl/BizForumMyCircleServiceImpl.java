package com.dooleen.service.biz.forum.my.circle.service.impl;
...
import com.dooleen.service.biz.forum.my.circle.mapper.BizForumMyCircleMapper;
...
public class BizForumMyCircleServiceImpl extends ServiceImpl<BizForumMyCircleMapper, BizForumMyCircleEntity> implements BizForumMyCircleService {
	...
	@Autowired
	private  BizForumMyCircleMapper bizForumMyCircleMapper;
	 ...
	 */
	//BIZ_FORUM_MY_CIRCLE("biz_forum_my_circle", "BizForumMyCircle", "AAAA"),


	private  static String seqPrefix= TableEntityORMEnum.BIZ_FORUM_MY_CIRCLE.getSeqPrefix();
	private  static String tableName = TableEntityORMEnum.BIZ_FORUM_MY_CIRCLE.getEntityName();
	...
	@Override
	@Cacheable(value = "bizForumMyCircle", key = "#commonMsg.head.tenantId+':'+#commonMsg.body.singleBody.id", condition = "#commonMsg.head.cacheAble eq 'true'")
	public CommonMsg<BizForumMyCircleEntity, NullEntity> queryBizForumMyCircle(CommonMsg<BizForumMyCircleEntity, NullEntity> commonMsg) {
		CommonUtil.service(commonMsg);
		List<NullEntity> nullEntityList = new ArrayList<NullEntity>();
		// 根据Key值查询
		BizForumMyCircleEntity bizForumMyCircleEntity = super.getById(commonMsg.getBody().getSingleBody().getId());
       
		commonMsg.getBody().setListBody(nullEntityList);
		
		BizResponseEnum.DATA_NOT_FOUND.assertNotNull(bizForumMyCircleEntity,commonMsg);

		//获取流程相关数据
		generalFlowProcessUtil.queryExtData(commonMsg);

		commonMsg.getBody().setSingleBody(bizForumMyCircleEntity);
		//设置返回值
	...
	@Override
	public CommonMsg<NullEntity, BizForumMyCircleEntity> queryBizForumMyCircleList(CommonMsg<NullEntity, BizForumMyCircleEntity> commonMsg) {
		CommonUtil.service(commonMsg);
		NullEntity nullEntity = new NullEntity();
		// 获取所有列表
		List<BizForumMyCircleEntity> bizForumMyCircleEntityList = super.list();
		commonMsg.getBody().setSingleBody(nullEntity);
		commonMsg.getBody().setListBody(bizForumMyCircleEntityList);
		//设置返回值
		CommonUtil.successReturn(commonMsg);
		return commonMsg;
	}
	...
	public CommonMsg<BizForumMyCircleEntity, BizForumMyCircleEntity> queryBizForumMyCircleListPage(
			CommonMsg<BizForumMyCircleEntity, BizForumMyCircleEntity> commonMsg) {
		CommonUtil.service(commonMsg); 
		...
		QueryWrapper<BizForumMyCircleEntity> queryWrapper = QueryWrapperUtil.parseWhereSql(sqlConditionList, BizForumMyCircleEntity.class);
		if (!isInSingleBody && commonMsg.getBody().getSqlCondition().size() > 0) {
			sqlConditionList.addAll(commonMsg.getBody().getSqlCondition());
			queryWrapper = QueryWrapperUtil.parseWhereSql(sqlConditionList, BizForumMyCircleEntity.class);
		}
		//处理OR查询条件
			...
			queryWrapper = QueryWrapperUtil.parseWhereOrSql(sqlConditionList, BizForumMyCircleEntity.class);
		}
		...
		// 定义分页查询
		Page<BizForumMyCircleEntity> pages = new Page<>();
		// 当前页
		pages.setCurrent(commonMsg.getBody().getCurrentPage());
		// 每页条数
		pages.setSize(commonMsg.getBody().getPageSize());
		IPage<BizForumMyCircleEntity> list =  super.page(pages, queryWrapper);
	...
	public CommonMsg<BizForumMyCircleEntity, BizForumMyCircleEntity> queryBizForumMyCircleListMap(
		...
		Collection<BizForumMyCircleEntity> bizForumMyCircleEntityList =  super.listByMap(conditionMap);
		List<BizForumMyCircleEntity> bizForumMyCircleMapList = new ArrayList<BizForumMyCircleEntity>(bizForumMyCircleEntityList);
		commonMsg.getBody().setListBody(bizForumMyCircleMapList);
		//设置返回值
		CommonUtil.successReturn(commonMsg);
		return commonMsg;
	}
	...
	@Override
	@CachePut(value = "bizForumMyCircle", key = "#commonMsg.head.tenantId+':'+#commonMsg.body.singleBody.id",condition = "#commonMsg.head.respCode eq 'S0000' and #commonMsg.head.cacheAble eq 'true'" )
	public CommonMsg<BizForumMyCircleEntity, NullEntity> saveBizForumMyCircle(CommonMsg<BizForumMyCircleEntity, NullEntity> commonMsg) {
		CommonUtil.service(commonMsg); 
		// 如果内容为空返回错误信息
	...
	@Override
	public CommonMsg<NullEntity, BizForumMyCircleEntity> saveBizForumMyCircleList(CommonMsg<NullEntity, BizForumMyCircleEntity> commonMsg) {
		CommonUtil.service(commonMsg); 
		// 如果listBody为空返回错误信息
	...
	@Override
	@CachePut(value = "bizForumMyCircle", key = "#commonMsg.head.tenantId+':'+#commonMsg.body.singleBody.id",condition = "#commonMsg.head.respCode eq 'S0000' and #commonMsg.head.cacheAble eq 'true'" )
	public CommonMsg<BizForumMyCircleEntity, NullEntity> updateBizForumMyCircle(CommonMsg<BizForumMyCircleEntity, NullEntity> commonMsg) {
		CommonUtil.service(commonMsg);
		// 如果内容为空返回错误信息
		if (! CommonUtil.commonMsgSingleBodyIsNotNull(commonMsg)) {
			return commonMsg;
		...
		// 根据Key值查询修改记录，需进行深拷贝！！
		log.debug("====开始调用查询方法 ====== ");
		CommonMsg<BizForumMyCircleEntity, NullEntity> queryCommonMsg = new CommonMsg<BizForumMyCircleEntity, NullEntity>();
		//定义body
		MutBean<BizForumMyCircleEntity, NullEntity> mutBean= new MutBean<BizForumMyCircleEntity, NullEntity>();
		...
		queryCommonMsg = SpringUtil.getBean(BizForumMyCircleServiceImpl.class).queryBizForumMyCircle(queryCommonMsg); 
	...
	@Override
	@CachePut(value = "bizForumMyCircle", key = "#commonMsg.head.tenantId+':'+#commonMsg.body.singleBody.id",condition = "#commonMsg.head.respCode eq 'S0000' and  #commonMsg.head.cacheAble eq 'true'" )
	public CommonMsg<BizForumMyCircleEntity, NullEntity> saveOrUpdateBizForumMyCircle(CommonMsg<BizForumMyCircleEntity, NullEntity> commonMsg) {
		CommonUtil.service(commonMsg);
		// 如果内容为空返回错误信息
		if (! CommonUtil.commonMsgSingleBodyIsNotNull(commonMsg)) {
			return commonMsg;
	...
	@Override
	public CommonMsg<NullEntity, BizForumMyCircleEntity> saveOrUpdateBizForumMyCircleList(CommonMsg<NullEntity, BizForumMyCircleEntity> commonMsg) {
		CommonUtil.service(commonMsg);
		// 如果内容为空返回错误信息
		if (!CommonUtil.commonMsgListBodyLength(commonMsg, CHECK_CONTENT)) {
			return commonMsg;
	...
	@Override
    @CacheEvict(value = "bizForumMyCircle", key = "#commonMsg.head.tenantId+':'+#commonMsg.body.singleBody.id", condition = "#commonMsg.head.cacheAble eq 'true'")
	public CommonMsg<BizForumMyCircleEntity, NullEntity> deleteBizForumMyCircle(CommonMsg<BizForumMyCircleEntity, NullEntity> commonMsg) {
		CommonUtil.service(commonMsg); 
	...
	@Override
	public CommonMsg<NullEntity, BizForumMyCircleEntity> deleteBizForumMyCircleList(CommonMsg<NullEntity, BizForumMyCircleEntity> commonMsg) {
		CommonUtil.service(commonMsg);
	...
	@Override
	public void exportBizForumMyCircleExcel(CommonMsg<BizForumMyCircleEntity, BizForumMyCircleEntity> commonMsg, HttpServletResponse response) {
		CommonUtil.service(commonMsg); 
		// 默认以传入的sqlCondition查询
		List<SQLConditionEntity> sqlConditionList = new ArrayList<SQLConditionEntity>();
		// 设定只查当前租户ID条件
		...
		if(list.getRecords().size() == 0){
			List<BizForumMyCircleEntity> listEntity = new ArrayList<BizForumMyCircleEntity>();
			listEntity.add(new BizForumMyCircleEntity());
		 ...
		 */
		Date start = new Date();
		String sheetName = "我的圈子表";
		String fileName = "我的圈子表-"+start.getTime();
		ExportParams params = new ExportParams();
		Workbook workbook = ExcelExportUtil.exportExcel(params, BizForumMyCircleEntity.class, commonMsg.getBody().getListBody());
		//导出Excel数据流
		try {
		...
		log.info("====exportBizForumMyCircleExcel service end == " + RespStatus.json.getString("S0000"));
	}
	/**
	* 导入excel新增
	*/
	@Override
	public CommonMsg<BizForumMyCircleEntity, NullEntity> uploadExcel(MultipartFile file, HttpServletRequest request) {
		...
		List<BizForumMyCircleEntity> list = new ArrayList<>();
		try {
			commonMsg.getHead().setRealName(decode(request.getHeader("realname").toString(),"UTF-8"));
			list = ExcelImportUtil.importExcel(file.getInputStream(), BizForumMyCircleEntity.class, params);
		} catch (Exception e) {
			e.printStackTrace();
		...
		//批量导入数据并做校验
		List<BizForumMyCircleEntity> tmpList = new ArrayList<>();
		for (int i = 0; i < list.size(); i++) {
			BizForumMyCircleEntity bizForumMyCircleEntity = list.get(i);

			//检查字段的有效性
			ValidationResult validationResult = ValidationUtils.validateEntity(bizForumMyCircleEntity);
			...
			if (isError == true) {
				...
				bizForumMyCircleEntity.setDataSign(DooleenMD5Util.md5(bizForumMyCircleEntity.toString(),  ConstantValue.md5Key));
				// 初始化数据
				bizForumMyCircleEntity = EntityInitUtils.initEntityPublicInfoForInsertOrUpdate(bizForumMyCircleEntity, commonMsg.getHead(),tableName, seqPrefix, redisTemplate);
				tmpList.add(bizForumMyCircleEntity);
			}
		}
		if(!map.isEmpty()){
			commonMsg.getHead().setRespCode(RespStatus.errorCode);
