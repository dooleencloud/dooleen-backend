package com.dooleen.service.system.micro.service.service.impl;
...
import com.dooleen.service.system.micro.service.mapper.SysMicroServiceMapper;
...
public class SysMicroServiceServiceImpl extends ServiceImpl<SysMicroServiceMapper, SysMicroServiceEntity> implements SysMicroServiceService {
	 ...
	 */
	//SYS_MICRO_SERVICE("sys_micro_service", "SysMicroService", "AAAA"),


	private  static String seqPrefix= TableEntityORMEnum.SYS_MICRO_SERVICE.getSeqPrefix();
	private  static String tableName = TableEntityORMEnum.SYS_MICRO_SERVICE.getEntityName();
	...
	@Override
	@Cacheable(value = "sysMicroService", key = "#commonMsg.head.tenantId+':'+#commonMsg.body.singleBody.id", condition = "#commonMsg.head.cacheAble eq 'true'")
	public CommonMsg<SysMicroServiceEntity, NullEntity> querySysMicroService(CommonMsg<SysMicroServiceEntity, NullEntity> commonMsg) {
		CommonUtil.service(commonMsg);
		List<NullEntity> nullEntityList = new ArrayList<NullEntity>();
		// 根据Key值查询
		SysMicroServiceEntity sysMicroServiceEntity = super.getById(commonMsg.getBody().getSingleBody().getId());
       
		commonMsg.getBody().setListBody(nullEntityList);
		
		BizResponseEnum.DATA_NOT_FOUND.assertNotNull(sysMicroServiceEntity,commonMsg);

		//获取流程相关数据
		generalFlowProcessUtil.queryExtData(commonMsg);

		commonMsg.getBody().setSingleBody(sysMicroServiceEntity);
		//设置返回值
	...
	@Override
	public CommonMsg<NullEntity, SysMicroServiceEntity> querySysMicroServiceList(CommonMsg<NullEntity, SysMicroServiceEntity> commonMsg) {
		CommonUtil.service(commonMsg);
		NullEntity nullEntity = new NullEntity();
		// 获取所有列表
		List<SysMicroServiceEntity> sysMicroServiceEntityList = super.list();
		commonMsg.getBody().setSingleBody(nullEntity);
		commonMsg.getBody().setListBody(sysMicroServiceEntityList);
		//设置返回值
		CommonUtil.successReturn(commonMsg);
		return commonMsg;
	}
	...
	@Override
	public CommonMsg<SysMicroServiceEntity, SysMicroServiceEntity> querySysMicroServiceListPage(CommonMsg<SysMicroServiceEntity, SysMicroServiceEntity> commonMsg) {
		CommonUtil.service(commonMsg); 
		...
		for (Map.Entry<String, Object> entry : singleBodyMap.entrySet()) {
			if(entry.getValue() != null && StringUtils.isNotBlank(entry.getValue().toString()))
			{
				if (!(StringUtil.isNumeric(entry.getValue().toString()) && (entry.getValue().equals("0")  || entry.getValue().equals("0.0") || entry.getValue().equals(0) || entry.getValue().equals(0.0)))) {
					...
					if(entry.getKey().equals("projectPackageName")){
						sqlConditionEntity0.setType("like");
					}
		...
		QueryWrapper<SysMicroServiceEntity> queryWrapper = QueryWrapperUtil.parseWhereSql(sqlConditionList, SysMicroServiceEntity.class);
		if (!isInSingleBody && commonMsg.getBody().getSqlCondition().size() > 0) {
			sqlConditionList.addAll(commonMsg.getBody().getSqlCondition());
			queryWrapper = QueryWrapperUtil.parseWhereSql(sqlConditionList, SysMicroServiceEntity.class);
		}
		//处理OR查询条件
			...
			queryWrapper = QueryWrapperUtil.parseWhereOrSql(sqlConditionList, SysMicroServiceEntity.class);
		}
		...
		// 定义分页查询
		Page<SysMicroServiceEntity> pages = new Page<>();
		// 当前页
		pages.setCurrent(commonMsg.getBody().getCurrentPage());
		// 每页条数
		pages.setSize(commonMsg.getBody().getPageSize());
		IPage<SysMicroServiceEntity> list =  super.page(pages, queryWrapper);
	...
	public CommonMsg<SysMicroServiceEntity, SysMicroServiceEntity> querySysMicroServiceListMap(
			CommonMsg<SysMicroServiceEntity, SysMicroServiceEntity> commonMsg) {
		CommonUtil.service(commonMsg); 
		...
		Collection<SysMicroServiceEntity> sysMicroServiceEntityList =  super.listByMap(conditionMap);
		List<SysMicroServiceEntity> sysMicroServiceMapList = new ArrayList<SysMicroServiceEntity>(sysMicroServiceEntityList);
		commonMsg.getBody().setListBody(sysMicroServiceMapList);
		//设置返回值
		CommonUtil.successReturn(commonMsg);
		return commonMsg;
	}
	...
	@Override
	@CachePut(value = "sysMicroService", key = "#commonMsg.head.tenantId+':'+#commonMsg.body.singleBody.id",condition = "#commonMsg.head.respCode eq 'S0000' and #commonMsg.head.cacheAble eq 'true'" )
	public CommonMsg<SysMicroServiceEntity, NullEntity> saveSysMicroService(CommonMsg<SysMicroServiceEntity, NullEntity> commonMsg) {
		CommonUtil.service(commonMsg); 
		// 如果内容为空返回错误信息
	...
	@Override
	public CommonMsg<NullEntity, SysMicroServiceEntity> saveSysMicroServiceList(CommonMsg<NullEntity, SysMicroServiceEntity> commonMsg) {
		CommonUtil.service(commonMsg); 
		// 如果listBody为空返回错误信息
	...
	@Override
	@CachePut(value = "sysMicroService", key = "#commonMsg.head.tenantId+':'+#commonMsg.body.singleBody.id",condition = "#commonMsg.head.respCode eq 'S0000' and #commonMsg.head.cacheAble eq 'true'" )
	public CommonMsg<SysMicroServiceEntity, NullEntity> updateSysMicroService(CommonMsg<SysMicroServiceEntity, NullEntity> commonMsg) {
		CommonUtil.service(commonMsg);
		// 如果内容为空返回错误信息
		if (! CommonUtil.commonMsgSingleBodyIsNotNull(commonMsg)) {
			return commonMsg;
		...
		// 根据Key值查询修改记录，需进行深拷贝！！
		log.debug("====开始调用查询方法 ====== ");
		CommonMsg<SysMicroServiceEntity, NullEntity> queryCommonMsg = new CommonMsg<SysMicroServiceEntity, NullEntity>();
		//定义body
		MutBean<SysMicroServiceEntity, NullEntity> mutBean= new MutBean<SysMicroServiceEntity, NullEntity>();
		...
		queryCommonMsg = SpringUtil.getBean(SysMicroServiceServiceImpl.class).querySysMicroService(queryCommonMsg); 
	...
	@Override
	@CachePut(value = "sysMicroService", key = "#commonMsg.head.tenantId+':'+#commonMsg.body.singleBody.id",condition = "#commonMsg.head.respCode eq 'S0000' and  #commonMsg.head.cacheAble eq 'true'" )
	public CommonMsg<SysMicroServiceEntity, NullEntity> saveOrUpdateSysMicroService(CommonMsg<SysMicroServiceEntity, NullEntity> commonMsg) {
		CommonUtil.service(commonMsg);
		// 如果内容为空返回错误信息
		if (! CommonUtil.commonMsgSingleBodyIsNotNull(commonMsg)) {
			return commonMsg;
	...
	@Override
	public CommonMsg<NullEntity, SysMicroServiceEntity> saveOrUpdateSysMicroServiceList(CommonMsg<NullEntity, SysMicroServiceEntity> commonMsg) {
		CommonUtil.service(commonMsg);
		// 如果内容为空返回错误信息
		if (!CommonUtil.commonMsgListBodyLength(commonMsg, CHECK_CONTENT)) {
			return commonMsg;
	...
	@Override
    @CacheEvict(value = "sysMicroService", key = "#commonMsg.head.tenantId+':'+#commonMsg.body.singleBody.id", condition = "#commonMsg.head.cacheAble eq 'true'")
	public CommonMsg<SysMicroServiceEntity, NullEntity> deleteSysMicroService(CommonMsg<SysMicroServiceEntity, NullEntity> commonMsg) {
		CommonUtil.service(commonMsg); 
	...
	@Override
	public CommonMsg<NullEntity, SysMicroServiceEntity> deleteSysMicroServiceList(CommonMsg<NullEntity, SysMicroServiceEntity> commonMsg) {
		CommonUtil.service(commonMsg);
	...
	@Override
	public void exportSysMicroServiceExcel(CommonMsg<SysMicroServiceEntity, SysMicroServiceEntity> commonMsg, HttpServletResponse response) {
		CommonUtil.service(commonMsg); 
		// 默认以传入的sqlCondition查询
		List<SQLConditionEntity> sqlConditionList = new ArrayList<SQLConditionEntity>();
		// 设定只查当前租户ID条件
		...
		if(list.getRecords().size() == 0){
			List<SysMicroServiceEntity> listEntity = new ArrayList<SysMicroServiceEntity>();
			listEntity.add(new SysMicroServiceEntity());
		 ...
		 */
		Date start = new Date();
		String sheetName = "微服务工程表";
		String fileName = "微服务工程表-"+start.getTime();
		ExportParams params = new ExportParams();
		Workbook workbook = ExcelExportUtil.exportExcel(params, SysMicroServiceEntity.class, commonMsg.getBody().getListBody());
		//导出Excel数据流
		try {
		...
		log.info("====exportSysMicroServiceExcel service end == " + RespStatus.json.getString("S0000"));
	}
	/**
	* 导入excel新增
	*/
	@Override
	public CommonMsg<SysMicroServiceEntity, NullEntity> uploadExcel(MultipartFile file, HttpServletRequest request) {
		...
		List<SysMicroServiceEntity> list = new ArrayList<>();
		try {
			commonMsg.getHead().setRealName(decode(request.getHeader("realname").toString(),"UTF-8"));
			list = ExcelImportUtil.importExcel(file.getInputStream(), SysMicroServiceEntity.class, params);
		} catch (Exception e) {
			e.printStackTrace();
		...
		//批量导入数据并做校验
		List<SysMicroServiceEntity> tmpList = new ArrayList<>();
		for (int i = 0; i < list.size(); i++) {
			SysMicroServiceEntity sysMicroServiceEntity = list.get(i);

			//检查字段的有效性
			ValidationResult validationResult = ValidationUtils.validateEntity(sysMicroServiceEntity);
			...
			if (isError == true) {
				...
				sysMicroServiceEntity.setDataSign(DooleenMD5Util.md5(sysMicroServiceEntity.toString(),  ConstantValue.md5Key));
				// 初始化数据
				sysMicroServiceEntity = EntityInitUtils.initEntityPublicInfoForInsertOrUpdate(sysMicroServiceEntity, commonMsg.getHead(),tableName, seqPrefix, redisTemplate);
				tmpList.add(sysMicroServiceEntity);
			}
		}
		if(!map.isEmpty()){
			commonMsg.getHead().setRespCode(RespStatus.errorCode);
