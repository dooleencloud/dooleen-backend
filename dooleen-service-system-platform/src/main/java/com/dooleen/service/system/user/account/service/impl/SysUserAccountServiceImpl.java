package com.dooleen.service.system.user.account.service.impl;
...
import com.dooleen.service.system.user.account.mapper.SysUserAccountMapper;
...
public class SysUserAccountServiceImpl extends ServiceImpl<SysUserAccountMapper, SysUserAccountEntity> implements SysUserAccountService {
	 ...
	 */
	//SYS_USER_ACCOUNT("sys_user_account", "SysUserAccount", "AAAA"),


	private  static String seqPrefix= TableEntityORMEnum.SYS_USER_ACCOUNT.getSeqPrefix();
	private  static String tableName = TableEntityORMEnum.SYS_USER_ACCOUNT.getEntityName();
	...
	@Override
	@Cacheable(value = "sysUserAccount", key = "#commonMsg.head.tenantId+':'+#commonMsg.body.singleBody.id", condition = "#commonMsg.head.cacheAble eq 'true'")
	public CommonMsg<SysUserAccountEntity, NullEntity> querySysUserAccount(CommonMsg<SysUserAccountEntity, NullEntity> commonMsg) {
		CommonUtil.service(commonMsg);
		List<NullEntity> nullEntityList = new ArrayList<NullEntity>();
		// 根据Key值查询
		SysUserAccountEntity sysUserAccountEntity = super.getById(commonMsg.getBody().getSingleBody().getId());
       
		commonMsg.getBody().setListBody(nullEntityList);
		
		BizResponseEnum.DATA_NOT_FOUND.assertNotNull(sysUserAccountEntity,commonMsg);

		//获取流程相关数据
		generalFlowProcessUtil.queryExtData(commonMsg);

		commonMsg.getBody().setSingleBody(sysUserAccountEntity);
		//设置返回值
	...
	@Override
	public CommonMsg<NullEntity, SysUserAccountEntity> querySysUserAccountList(CommonMsg<NullEntity, SysUserAccountEntity> commonMsg) {
		CommonUtil.service(commonMsg);
		NullEntity nullEntity = new NullEntity();
		// 获取所有列表
		List<SysUserAccountEntity> sysUserAccountEntityList = super.list();
		commonMsg.getBody().setSingleBody(nullEntity);
		commonMsg.getBody().setListBody(sysUserAccountEntityList);
		//设置返回值
		CommonUtil.successReturn(commonMsg);
		return commonMsg;
	}
	...
	@Override
	public CommonMsg<SysUserAccountEntity, SysUserAccountEntity> querySysUserAccountListPage(CommonMsg<SysUserAccountEntity, SysUserAccountEntity> commonMsg) {
		CommonUtil.service(commonMsg); 
		...
		QueryWrapper<SysUserAccountEntity> queryWrapper = QueryWrapperUtil.parseWhereSql(sqlConditionList, SysUserAccountEntity.class);
		if (!isInSingleBody && commonMsg.getBody().getSqlCondition().size() > 0) {
			sqlConditionList.addAll(commonMsg.getBody().getSqlCondition());
			queryWrapper = QueryWrapperUtil.parseWhereSql(sqlConditionList, SysUserAccountEntity.class);
		}
		//处理OR查询条件
			...
			queryWrapper = QueryWrapperUtil.parseWhereOrSql(sqlConditionList, SysUserAccountEntity.class);
		}
		...
		// 定义分页查询
		Page<SysUserAccountEntity> pages = new Page<>();
		// 当前页
		pages.setCurrent(commonMsg.getBody().getCurrentPage());
		// 每页条数
		pages.setSize(commonMsg.getBody().getPageSize());
		IPage<SysUserAccountEntity> list =  super.page(pages, queryWrapper);
	...
	public CommonMsg<SysUserAccountEntity, SysUserAccountEntity> querySysUserAccountListMap(
			CommonMsg<SysUserAccountEntity, SysUserAccountEntity> commonMsg) {
		CommonUtil.service(commonMsg); 
		...
		Collection<SysUserAccountEntity> sysUserAccountEntityList =  super.listByMap(conditionMap);
		List<SysUserAccountEntity> sysUserAccountMapList = new ArrayList<SysUserAccountEntity>(sysUserAccountEntityList);
		commonMsg.getBody().setListBody(sysUserAccountMapList);
		//设置返回值
		CommonUtil.successReturn(commonMsg);
		return commonMsg;
	}
	...
	@Override
	@CachePut(value = "sysUserAccount", key = "#commonMsg.head.tenantId+':'+#commonMsg.body.singleBody.id",condition = "#commonMsg.head.respCode eq 'S0000' and #commonMsg.head.cacheAble eq 'true'" )
	public CommonMsg<SysUserAccountEntity, NullEntity> saveSysUserAccount(CommonMsg<SysUserAccountEntity, NullEntity> commonMsg) {
		CommonUtil.service(commonMsg); 
		// 如果内容为空返回错误信息
	...
	@Override
	public CommonMsg<NullEntity, SysUserAccountEntity> saveSysUserAccountList(CommonMsg<NullEntity, SysUserAccountEntity> commonMsg) {
		CommonUtil.service(commonMsg); 
		// 如果listBody为空返回错误信息
	...
	@Override
	@CachePut(value = "sysUserAccount", key = "#commonMsg.head.tenantId+':'+#commonMsg.body.singleBody.id",condition = "#commonMsg.head.respCode eq 'S0000' and #commonMsg.head.cacheAble eq 'true'" )
	public CommonMsg<SysUserAccountEntity, NullEntity> updateSysUserAccount(CommonMsg<SysUserAccountEntity, NullEntity> commonMsg) {
		CommonUtil.service(commonMsg);
		// 如果内容为空返回错误信息
		if (! CommonUtil.commonMsgSingleBodyIsNotNull(commonMsg)) {
			return commonMsg;
		...
		// 根据Key值查询修改记录，需进行深拷贝！！
		log.debug("====开始调用查询方法 ====== ");
		CommonMsg<SysUserAccountEntity, NullEntity> queryCommonMsg = new CommonMsg<SysUserAccountEntity, NullEntity>();
		//定义body
		MutBean<SysUserAccountEntity, NullEntity> mutBean= new MutBean<SysUserAccountEntity, NullEntity>();
		...
		queryCommonMsg = SpringUtil.getBean(SysUserAccountServiceImpl.class).querySysUserAccount(queryCommonMsg); 
	...
	@Override
	@CachePut(value = "sysUserAccount", key = "#commonMsg.head.tenantId+':'+#commonMsg.body.singleBody.id",condition = "#commonMsg.head.respCode eq 'S0000' and  #commonMsg.head.cacheAble eq 'true'" )
	public CommonMsg<SysUserAccountEntity, NullEntity> saveOrUpdateSysUserAccount(CommonMsg<SysUserAccountEntity, NullEntity> commonMsg) {
		CommonUtil.service(commonMsg);
		// 如果内容为空返回错误信息
		if (! CommonUtil.commonMsgSingleBodyIsNotNull(commonMsg)) {
			return commonMsg;
	...
	@Override
	public CommonMsg<NullEntity, SysUserAccountEntity> saveOrUpdateSysUserAccountList(CommonMsg<NullEntity, SysUserAccountEntity> commonMsg) {
		CommonUtil.service(commonMsg);
		// 如果内容为空返回错误信息
		if (!CommonUtil.commonMsgListBodyLength(commonMsg, CHECK_CONTENT)) {
			return commonMsg;
	...
	@Override
    @CacheEvict(value = "sysUserAccount", key = "#commonMsg.head.tenantId+':'+#commonMsg.body.singleBody.id", condition = "#commonMsg.head.cacheAble eq 'true'")
	public CommonMsg<SysUserAccountEntity, NullEntity> deleteSysUserAccount(CommonMsg<SysUserAccountEntity, NullEntity> commonMsg) {
		CommonUtil.service(commonMsg); 
	...
	@Override
	public CommonMsg<NullEntity, SysUserAccountEntity> deleteSysUserAccountList(CommonMsg<NullEntity, SysUserAccountEntity> commonMsg) {
		CommonUtil.service(commonMsg);
	...
	@Override
	public void exportSysUserAccountExcel(CommonMsg<SysUserAccountEntity, SysUserAccountEntity> commonMsg, HttpServletResponse response) {
		CommonUtil.service(commonMsg); 
		// 默认以传入的sqlCondition查询
		List<SQLConditionEntity> sqlConditionList = new ArrayList<SQLConditionEntity>();
		// 设定只查当前租户ID条件
		...
		if(list.getRecords().size() == 0){
			List<SysUserAccountEntity> listEntity = new ArrayList<SysUserAccountEntity>();
			listEntity.add(new SysUserAccountEntity());
		 ...
		 */
		Date start = new Date();
		String sheetName = "用户账户表";
		String fileName = "用户账户表-"+start.getTime();
		ExportParams params = new ExportParams();
		Workbook workbook = ExcelExportUtil.exportExcel(params, SysUserAccountEntity.class, commonMsg.getBody().getListBody());
		//导出Excel数据流
		try {
		...
		log.info("====exportSysUserAccountExcel service end == " + RespStatus.json.getString("S0000"));
	}
	/**
	* 导入excel新增
	*/
	@Override
	public CommonMsg<SysUserAccountEntity, NullEntity> uploadExcel(MultipartFile file, HttpServletRequest request) {
		...
		List<SysUserAccountEntity> list = new ArrayList<>();
		try {
			commonMsg.getHead().setRealName(decode(request.getHeader("realname").toString(),"UTF-8"));
			list = ExcelImportUtil.importExcel(file.getInputStream(), SysUserAccountEntity.class, params);
		} catch (Exception e) {
			e.printStackTrace();
		...
		//批量导入数据并做校验
		List<SysUserAccountEntity> tmpList = new ArrayList<>();
		for (int i = 0; i < list.size(); i++) {
			SysUserAccountEntity sysUserAccountEntity = list.get(i);

			//检查字段的有效性
			ValidationResult validationResult = ValidationUtils.validateEntity(sysUserAccountEntity);
			...
			if (isError == true) {
				...
				sysUserAccountEntity.setDataSign(DooleenMD5Util.md5(sysUserAccountEntity.toString(),  ConstantValue.md5Key));
				// 初始化数据
				sysUserAccountEntity = EntityInitUtils.initEntityPublicInfoForInsertOrUpdate(sysUserAccountEntity, commonMsg.getHead(),tableName, seqPrefix, redisTemplate);
				tmpList.add(sysUserAccountEntity);
			}
		}
		if(!map.isEmpty()){
			commonMsg.getHead().setRespCode(RespStatus.errorCode);
