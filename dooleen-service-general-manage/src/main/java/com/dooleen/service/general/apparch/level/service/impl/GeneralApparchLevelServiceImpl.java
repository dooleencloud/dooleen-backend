package com.dooleen.service.general.apparch.level.service.impl;
...
import com.dooleen.service.general.apparch.level.mapper.GeneralApparchLevelMapper;
import com.dooleen.service.general.apparch.level.service.GeneralApparchLevelService;
...
public class GeneralApparchLevelServiceImpl extends ServiceImpl<GeneralApparchLevelMapper, GeneralApparchLevelEntity> implements GeneralApparchLevelService {
	...
	@Autowired
	private  GeneralApparchLevelMapper generalApparchLevelMapper;
	
	@Autowired
	private  GetBizParamsService getBizParamsService; 

	private  static String seqPrefix= TableEntityORMEnum.GENERAL_APPARCH_LEVEL.getSeqPrefix();
	private  static String tableName = TableEntityORMEnum.GENERAL_APPARCH_LEVEL.getEntityName();

	@Autowired
	private RedisTemplate<String, ?> redisTemplate;
	@Override
	@Cacheable(value = "generalApparchLevel", key = "#commonMsg.head.tenantId+':'+#commonMsg.body.singleBody.id", condition = "#commonMsg.head.cacheAble eq 'true'")
	public CommonMsg<GeneralApparchLevelEntity, NullEntity> queryGeneralApparchLevel(CommonMsg<GeneralApparchLevelEntity, NullEntity> commonMsg) {
		CommonUtil.service(commonMsg);
		List<NullEntity> nullEntityList = new ArrayList<NullEntity>();
		// 根据Key值查询
		GeneralApparchLevelEntity generalApparchLevelEntity = super.getById(commonMsg.getBody().getSingleBody().getId());
       
		commonMsg.getBody().setListBody(nullEntityList);
		
		BizResponseEnum.DATA_NOT_FOUND.assertNotNull(generalApparchLevelEntity,commonMsg);
		/** 不需要流程请自行删除  开始 */
		//判断是否需要查流程信息
		...
		commonMsg.getBody().setSingleBody(generalApparchLevelEntity);
		//设置返回值
	...
	@Override
	public CommonMsg<NullEntity, GeneralApparchLevelEntity> queryGeneralApparchLevelList(CommonMsg<NullEntity, GeneralApparchLevelEntity> commonMsg) {
		CommonUtil.service(commonMsg);
		NullEntity nullEntity = new NullEntity();
		// 获取所有列表
		List<GeneralApparchLevelEntity> generalApparchLevelEntityList = super.list();
		commonMsg.getBody().setSingleBody(nullEntity);
		commonMsg.getBody().setListBody(generalApparchLevelEntityList);
		//设置返回值
		CommonUtil.successReturn(commonMsg);
		return commonMsg;
	}
	...
	@Override
	public CommonMsg<GeneralApparchLevelEntity, GeneralApparchLevelEntity> queryGeneralApparchLevelListPage(
			CommonMsg<GeneralApparchLevelEntity, GeneralApparchLevelEntity> commonMsg) {
		CommonUtil.service(commonMsg); 
		...
		QueryWrapper<GeneralApparchLevelEntity> queryWrapper = QueryWrapperUtil.parseWhereSql(sqlConditionList, GeneralApparchLevelEntity.class);
		if (!isInSingleBody && commonMsg.getBody().getSqlCondition().size() > 0) {
			sqlConditionList.addAll(commonMsg.getBody().getSqlCondition());
			queryWrapper = QueryWrapperUtil.parseWhereSql(sqlConditionList, GeneralApparchLevelEntity.class);
		}
		//处理OR查询条件
			...
			queryWrapper = QueryWrapperUtil.parseWhereOrSql(sqlConditionList, GeneralApparchLevelEntity.class);
		}
		...
		// 定义分页查询
		Page<GeneralApparchLevelEntity> pages = new Page<>();
		// 当前页
		pages.setCurrent(commonMsg.getBody().getCurrentPage());
		// 每页条数
		pages.setSize(commonMsg.getBody().getPageSize());
		IPage<GeneralApparchLevelEntity> list =  super.page(pages, queryWrapper);
	...
	@Override
	public CommonMsg<GeneralApparchLevelEntity, GeneralApparchLevelEntity> queryGeneralApparchLevelListMap(
			CommonMsg<GeneralApparchLevelEntity, GeneralApparchLevelEntity> commonMsg) {
		CommonUtil.service(commonMsg); 
		...
		Collection<GeneralApparchLevelEntity> generalApparchLevelEntityList =  super.listByMap(conditionMap);
		List<GeneralApparchLevelEntity> generalApparchLevelMapList = new ArrayList<GeneralApparchLevelEntity>(generalApparchLevelEntityList);
		commonMsg.getBody().setListBody(generalApparchLevelMapList);
		//设置返回值
		CommonUtil.successReturn(commonMsg);
		return commonMsg;
	}
	...
	@Override
	@CachePut(value = "generalApparchLevel", key = "#commonMsg.head.tenantId+':'+#commonMsg.body.singleBody.id",condition = "#commonMsg.head.respCode eq 'S0000' and  #commonMsg.head.cacheAble eq 'true'" )
	public CommonMsg<GeneralApparchLevelEntity, NullEntity> saveGeneralApparchLevel(CommonMsg<GeneralApparchLevelEntity, NullEntity> commonMsg) {
		CommonUtil.service(commonMsg); 
		// 如果内容为空返回错误信息
	...
	@Override
	public CommonMsg<NullEntity, GeneralApparchLevelEntity> saveGeneralApparchLevelList(CommonMsg<NullEntity, GeneralApparchLevelEntity> commonMsg) {
		CommonUtil.service(commonMsg); 
		// 如果listBody为空返回错误信息
	...
	@Override
	@CachePut(value = "generalApparchLevel", key = "#commonMsg.head.tenantId+':'+#commonMsg.body.singleBody.id",condition = "#commonMsg.head.respCode eq 'S0000' and  #commonMsg.head.cacheAble eq 'true'" )
	public CommonMsg<GeneralApparchLevelEntity, NullEntity> updateGeneralApparchLevel(CommonMsg<GeneralApparchLevelEntity, NullEntity> commonMsg) {
		CommonUtil.service(commonMsg);
		// 如果内容为空返回错误信息
		if (! CommonUtil.commonMsgSingleBodyIsNotNull(commonMsg)) {
			return commonMsg;
		...
		// 根据Key值查询修改记录，需进行深拷贝！！
		log.debug("====开始调用查询方法 ====== ");
		CommonMsg<GeneralApparchLevelEntity, NullEntity> queryCommonMsg = new CommonMsg<GeneralApparchLevelEntity, NullEntity>();
		//定义singleBody
		GeneralApparchLevelEntity singleBody = new GeneralApparchLevelEntity();
		singleBody.setId(commonMsg.getBody().getSingleBody().getId());
		//定义body
		MutBean<GeneralApparchLevelEntity, NullEntity> mutBean= new MutBean<GeneralApparchLevelEntity, NullEntity>();
		...
		queryCommonMsg = SpringUtil.getBean(GeneralApparchLevelServiceImpl.class).queryGeneralApparchLevel(queryCommonMsg); 
	...
	@Override
	@CachePut(value = "generalApparchLevel", key = "#commonMsg.head.tenantId+':'+#commonMsg.body.singleBody.id",condition = "#commonMsg.head.respCode eq 'S0000' and  #commonMsg.head.cacheAble eq 'true'" )
	public CommonMsg<GeneralApparchLevelEntity, NullEntity> saveOrUpdateGeneralApparchLevel(CommonMsg<GeneralApparchLevelEntity, NullEntity> commonMsg) {
		CommonUtil.service(commonMsg);
		// 如果内容为空返回错误信息
		if (! CommonUtil.commonMsgSingleBodyIsNotNull(commonMsg)) {
			return commonMsg;
	...
	@Override
	public CommonMsg<NullEntity, GeneralApparchLevelEntity> saveOrUpdateGeneralApparchLevelList(CommonMsg<NullEntity, GeneralApparchLevelEntity> commonMsg) {
		CommonUtil.service(commonMsg);
		// 如果内容为空返回错误信息
		if (!CommonUtil.commonMsgListBodyLength(commonMsg, CHECK_CONTENT)) {
			return commonMsg;
	...
	@Override
    @CacheEvict(value = "generalApparchLevel", key = "#commonMsg.head.tenantId+':'+#commonMsg.body.singleBody.id", condition = "#commonMsg.head.cacheAble eq 'true'")
	public CommonMsg<GeneralApparchLevelEntity, NullEntity> deleteGeneralApparchLevel(CommonMsg<GeneralApparchLevelEntity, NullEntity> commonMsg) {
		CommonUtil.service(commonMsg); 
	...
	@Override
	public CommonMsg<NullEntity, GeneralApparchLevelEntity> deleteGeneralApparchLevelList(CommonMsg<NullEntity, GeneralApparchLevelEntity> commonMsg) {
		CommonUtil.service(commonMsg);
	...
	@Override
	public void exportGeneralApparchLevelExcel(CommonMsg<GeneralApparchLevelEntity, GeneralApparchLevelEntity> commonMsg, HttpServletResponse response) {
		CommonUtil.service(commonMsg); 
		// 默认以传入的sqlCondition查询
		List<SQLConditionEntity> sqlConditionList = new ArrayList<SQLConditionEntity>();
		// 设定只查当前租户ID条件
		 ...
		 * 生成excel并输出
		 */
		String sheetName = "应用架构层级表";
		String fileName = "应用架构层级表";
		...
		for (GeneralApparchLevelEntity generalApparchLevelEntity: commonMsg.getBody().getListBody()) {
			List<String> lists= new ArrayList<String>();
			lists.add(generalApparchLevelEntity.getArchitectureLevel());	    		
			lists.add(generalApparchLevelEntity.getAppGroup());	    		
			lists.add(generalApparchLevelEntity.getFunctionGroup());	    		
			lists.add(generalApparchLevelEntity.getBelongDeptName());	    		
			lists.add(generalApparchLevelEntity.getRespsbUserName());	    		
			excelData.add(lists);
		} 
		...
		log.info("====exportGeneralApparchLevelExcel service end == " + RespStatus.json.getString("S0000"));
	}
}
