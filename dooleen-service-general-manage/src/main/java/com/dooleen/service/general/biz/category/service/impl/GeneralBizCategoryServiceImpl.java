package com.dooleen.service.general.biz.category.service.impl;
...
import com.dooleen.service.general.biz.category.mapper.GeneralBizCategoryMapper;
...
public class GeneralBizCategoryServiceImpl extends ServiceImpl<GeneralBizCategoryMapper, GeneralBizCategoryEntity> implements GeneralBizCategoryService {
	...
	@Autowired
	private  GeneralBizCategoryMapper generalBizCategoryMapper;
	 ...
	 */
	//GENERAL_BIZ_CATEGORY("general_biz_category", "GeneralBizCategory", "AAAA"),


	private  static String seqPrefix= TableEntityORMEnum.GENERAL_BIZ_CATEGORY.getSeqPrefix();
	private  static String tableName = TableEntityORMEnum.GENERAL_BIZ_CATEGORY.getEntityName();

	@Autowired
	private RedisTemplate<String, ?> redisTemplate;
	@Override
	@Cacheable(value = "generalBizCategory", key = "#commonMsg.head.tenantId+':'+#commonMsg.body.singleBody.id")
	public CommonMsg<GeneralBizCategoryEntity, NullEntity> queryGeneralBizCategory(CommonMsg<GeneralBizCategoryEntity, NullEntity> commonMsg) {
		CommonUtil.service(commonMsg);
		List<NullEntity> nullEntityList = new ArrayList<NullEntity>();
		// 根据Key值查询
		GeneralBizCategoryEntity generalBizCategoryEntity = super.getById(commonMsg.getBody().getSingleBody().getId());
       
		commonMsg.getBody().setListBody(nullEntityList);
		
		BizResponseEnum.DATA_NOT_FOUND.assertNotNull(generalBizCategoryEntity,commonMsg);
		/** 不需要流程请自行删除  开始 */
		//判断是否需要查流程信息
		...
		commonMsg.getBody().setSingleBody(generalBizCategoryEntity);
		//设置返回值
	...
	@Override
	public CommonMsg<NullEntity, GeneralBizCategoryEntity> queryGeneralBizCategoryList(CommonMsg<NullEntity, GeneralBizCategoryEntity> commonMsg) {
		CommonUtil.service(commonMsg);
		NullEntity nullEntity = new NullEntity();
		// 获取所有列表
		List<GeneralBizCategoryEntity> generalBizCategoryEntityList = super.list();
		commonMsg.getBody().setSingleBody(nullEntity);
		commonMsg.getBody().setListBody(generalBizCategoryEntityList);
		//设置返回值
		CommonUtil.successReturn(commonMsg);
		return commonMsg;
	}
	...
	public CommonMsg<GeneralBizCategoryEntity, GeneralBizCategoryEntity> queryGeneralBizCategoryListPage(
			CommonMsg<GeneralBizCategoryEntity, GeneralBizCategoryEntity> commonMsg) {
		CommonUtil.service(commonMsg); 
		...
		QueryWrapper<GeneralBizCategoryEntity> queryWrapper = QueryWrapperUtil.parseWhereSql(sqlConditionList, GeneralBizCategoryEntity.class);
		if (!isInSingleBody && commonMsg.getBody().getSqlCondition().size() > 0) {
			sqlConditionList.addAll(commonMsg.getBody().getSqlCondition());
			queryWrapper = QueryWrapperUtil.parseWhereSql(sqlConditionList, GeneralBizCategoryEntity.class);
		}
		//处理OR查询条件
			...
			queryWrapper = QueryWrapperUtil.parseWhereOrSql(sqlConditionList, GeneralBizCategoryEntity.class);
		}
		...
		// 定义分页查询
		Page<GeneralBizCategoryEntity> pages = new Page<>();
		// 当前页
		pages.setCurrent(commonMsg.getBody().getCurrentPage());
		// 每页条数
		pages.setSize(commonMsg.getBody().getPageSize());
		IPage<GeneralBizCategoryEntity> list =  super.page(pages, queryWrapper);
	...
	public CommonMsg<GeneralBizCategoryEntity, GeneralBizCategoryEntity> queryGeneralBizCategoryListMap(
		...
		Collection<GeneralBizCategoryEntity> generalBizCategoryEntityList =  super.listByMap(conditionMap);
		List<GeneralBizCategoryEntity> generalBizCategoryMapList = new ArrayList<GeneralBizCategoryEntity>(generalBizCategoryEntityList);
		commonMsg.getBody().setListBody(generalBizCategoryMapList);
		//设置返回值
		CommonUtil.successReturn(commonMsg);
		return commonMsg;
	}
	...
	@Override
	@CachePut(value = "generalBizCategory", key = "#commonMsg.head.tenantId+':'+#commonMsg.body.singleBody.id",condition = "#commonMsg.head.respCode eq 'S0000'")
	public CommonMsg<GeneralBizCategoryEntity, NullEntity> saveGeneralBizCategory(CommonMsg<GeneralBizCategoryEntity, NullEntity> commonMsg) {
		CommonUtil.service(commonMsg); 
		// 如果内容为空返回错误信息
	...
	@Override
	public CommonMsg<NullEntity, GeneralBizCategoryEntity> saveGeneralBizCategoryList(CommonMsg<NullEntity, GeneralBizCategoryEntity> commonMsg) {
		CommonUtil.service(commonMsg); 
		// 如果listBody为空返回错误信息
	...
	@Override
	@CachePut(value = "generalBizCategory", key = "#commonMsg.head.tenantId+':'+#commonMsg.body.singleBody.id",condition = "#commonMsg.head.respCode eq 'S0000'")
	public CommonMsg<GeneralBizCategoryEntity, NullEntity> updateGeneralBizCategory(CommonMsg<GeneralBizCategoryEntity, NullEntity> commonMsg) {
		CommonUtil.service(commonMsg);
		// 如果内容为空返回错误信息
		if (! CommonUtil.commonMsgSingleBodyIsNotNull(commonMsg)) {
			return commonMsg;
		...
		// 根据Key值查询修改记录，需进行深拷贝！！
		log.debug("====开始调用查询方法 ====== ");
		CommonMsg<GeneralBizCategoryEntity, NullEntity> queryCommonMsg = new CommonMsg<GeneralBizCategoryEntity, NullEntity>();
		//定义singleBody
		GeneralBizCategoryEntity singleBody = new GeneralBizCategoryEntity();
		singleBody.setId(commonMsg.getBody().getSingleBody().getId());
		//定义body
		MutBean<GeneralBizCategoryEntity, NullEntity> mutBean= new MutBean<GeneralBizCategoryEntity, NullEntity>();
		...
		queryCommonMsg = SpringUtil.getBean(GeneralBizCategoryServiceImpl.class).queryGeneralBizCategory(queryCommonMsg); 
	...
	@Override
	@CachePut(value = "generalBizCategory", key = "#commonMsg.head.tenantId+':'+#commonMsg.body.singleBody.id",condition = "#commonMsg.head.respCode eq 'S0000'")
	public CommonMsg<GeneralBizCategoryEntity, NullEntity> saveOrUpdateGeneralBizCategory(CommonMsg<GeneralBizCategoryEntity, NullEntity> commonMsg) {
		CommonUtil.service(commonMsg);
		// 如果内容为空返回错误信息
		if (! CommonUtil.commonMsgSingleBodyIsNotNull(commonMsg)) {
			return commonMsg;
	...
	@Override
	public CommonMsg<NullEntity, GeneralBizCategoryEntity> saveOrUpdateGeneralBizCategoryList(CommonMsg<NullEntity, GeneralBizCategoryEntity> commonMsg) {
		CommonUtil.service(commonMsg);
		// 如果内容为空返回错误信息
		if (!CommonUtil.commonMsgListBodyLength(commonMsg, CHECK_CONTENT)) {
			return commonMsg;
	...
	@Override
    @CacheEvict(value = "generalBizCategory", key = "#commonMsg.head.tenantId+':'+#commonMsg.body.singleBody.id")
	public CommonMsg<GeneralBizCategoryEntity, NullEntity> deleteGeneralBizCategory(CommonMsg<GeneralBizCategoryEntity, NullEntity> commonMsg) {
		CommonUtil.service(commonMsg); 
	...
	@Override
	public CommonMsg<NullEntity, GeneralBizCategoryEntity> deleteGeneralBizCategoryList(CommonMsg<NullEntity, GeneralBizCategoryEntity> commonMsg) {
		CommonUtil.service(commonMsg);
	...
	@Override
	public void exportGeneralBizCategoryExcel(CommonMsg<GeneralBizCategoryEntity, GeneralBizCategoryEntity> commonMsg, HttpServletResponse response) {
		CommonUtil.service(commonMsg); 
		// 默认以传入的sqlCondition查询
		List<SQLConditionEntity> sqlConditionList = new ArrayList<SQLConditionEntity>();
		// 设定只查当前租户ID条件
		 ...
		 * 生成excel并输出
		 */
		String sheetName = "业务种类";
		String fileName = "业务种类";
		...
		for (GeneralBizCategoryEntity generalBizCategoryEntity: commonMsg.getBody().getListBody()) {
			List<String> lists= new ArrayList<String>();
			lists.add(generalBizCategoryEntity.getId());	    		
			lists.add(generalBizCategoryEntity.getTenantId());	    		
			lists.add(generalBizCategoryEntity.getBizLines());	    		
			lists.add(generalBizCategoryEntity.getBizPlate());	    		
			lists.add(generalBizCategoryEntity.getBizCategory());	    		
			lists.add(generalBizCategoryEntity.getBelongDeptName());	    		
			lists.add(generalBizCategoryEntity.getRespsbUserName());	    		
			lists.add(generalBizCategoryEntity.getRespsbRealName());	    		
			excelData.add(lists);
		} 
		...
		log.info("====exportGeneralBizCategoryExcel service end == " + RespStatus.json.getString("S0000"));
	}
}
